        ...
        return {
            ...
            generateVideo: async (prompt, options) => {
                const config = {
                    ...
                    ...(options?.size ? { size: options.size, videoSize: options.size } : {}),
                    ...(options?.seconds ? { videoSeconds: options.seconds } : {}),
                };
                ensureReady(config);
                const file = await storeGeneratedVideo(await requestVideoGeneration(config, { prompt, images: toReferences(options?.references), videos: [], audios: [] }, { signal: options?.signal }));
