...
def main() -> None:
    ...
    source_root = canvas_dir / "web/src"
    for path in source_root.rglob("*"):
        if path.suffix not in {".ts", ".tsx", ".js", ".jsx"} or ".git" in path.parts:
            continue
        source = path.read_text(encoding="utf-8", errors="replace")
        for domain in SUPPORTED_DOMAINS:
            if domain != selected_domain and f"https://{domain}" in source:
                fail(f"source contains forbidden domain {domain}: {path}")
