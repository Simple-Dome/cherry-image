...
+ [新增] Image 发布改为单一 `main` 集成线下的三域名 release matrix，为每个域名创建独立 release ref、detached source pair 和构建期域名参数。
+ [调整] Image 发布范围覆盖所选 profile 的 uploads edge，并以 Shell、Canvas、uploads 双色与隔离 MinIO 拓扑作为发布 profile 的就绪前提。
...
+ [调整] Shell 与 Canvas 通过构建期 `VITE_FIXED_API_BASE_URL` 绑定所选域名，浏览器配置、导入文件和分享 URL 均不能覆盖。
...
+ [新增] 模型渠道新增「431 即梦」协议，统一视频设置会按 Leonardo Seedance 模型展示合法参数，并支持真实参考素材、异步查询与鉴权下载。
+ [新增] 模型渠道新增「933 即梦」协议，支持 Firefly Video v2 纯文本与真实参考素材任务、异步查询及带鉴权视频下载。
+ [新增] 画布新增结构化分镜节点与图片首尾帧角色，支持 933 即梦视频生成前校验、角色常驻标识和输入摘要。
+ [调整] 统一视频设置新增独立横屏尺寸、声音与固定 Seed 开关、480p/720p/1080p 清晰度，并在 933 Fast 模型下禁用 1080p，默认时长改为 5 秒。
+ [修复] 933 即梦提示词、负面提示词及分镜提示词不再受前端 1500 字符限制，交由上游处理。
...
+ [修复] 补充分镜工具栏中文提示，并恢复分镜节点标题栏和空白区域拖动。
